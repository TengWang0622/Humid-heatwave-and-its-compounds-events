...
# Description: Identify and tabulate all the compound events (Humid heatwave with following precipitation) dataframe, 
...
df$checkIndex_Precip=ifelse(df$precip >= 0.001 & df$precip < 0.197, 1, 0) ##mild
#df$checkIndex_Precip=ifelse(df$precip >= 0.197 & df$precip < 0.669, 1, 0) ##light precipitation
...
df$checkCompound=df$checkEvent*df$checkPrecip
id_row=seq(1,length(df$checkCompound),by=No_subgroup)# -------------Modify the variable to consider heatwave，precipitation or compound: df$checkEvent, df$checkPrecip, df$checkCompound
Po_row=which(df$checkCompound[id_row]==1)
...
saveRDS(prepost,'/Users/wangteng/Downloads/ChinaCompS.rds')
